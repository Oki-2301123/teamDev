...
try {
        ...
        if (isset($_POST['delete_shohin'])) {
            ...
            foreach ($delete_shohin as $a => $shohins_id) {
                ...
                if (!$stmt->execute([$shohins_id, $_SESSION['cart_id']])) {
                    $_SESSION['err'] = "商品の削除に失敗しました（商品ID: $shohins_id）";
                }
            }
        }
    } else {
    ...
    $stmt = $pdo->prepare($sql);
    if (!$stmt->execute([$_SESSION['cart_id']])) {
        $_SESSION['err'] = "カート合計金額の計算に失敗しました。";
    }
    ...
    if (!$stmt->execute([$total, $_SESSION['cart_id']])) {
        $_SESSION['err'] = "カート合計金額の更新に失敗しました。";
    }

    // 成功メッセージをセットしてリダイレクト
    $_SESSION['msg'] = 'カートが正常に更新されました。';
    header('Location: cart.php');
    exit;
} catch (Exception $e) {
    // エラーメッセージをセッションに保存
    $_SESSION['err'] = '問題が発生しました';
    header('Location: toppages.php');
    exit;
}
